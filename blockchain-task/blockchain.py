...
MINING_REWARD = 10
genesis_block = {'previous_hash': '', 'index': 0, 'transictions': [], 'proof': 100}
...
def proof_of_work():
    ...
    proof = 0
    while not valid_proof(open_transations, last_hash, proof):
        proof+= 1
    return proof
...
def mine_block():
    ...
    block = {'previous_hash': hashed_block, 'index': len(blockchain), 'transictions': copied_transaction, 'proof': proof}
...
def verify_chain():
    for (index, block) in enumerate(blockchain): # enumerate() gives back a tuple which contains two pieces of information: index and element
        ...
        if block['previous_hash'] != hash_block(blockchain[index - 1]):
            return False
        if not valid_proof(block['transictions'][:-1], block['previous_hash'], block['proof']):
